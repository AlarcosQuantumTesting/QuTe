...
""");
    ...
    public static String buildStochasticScript(
        ...
        sb.append("outputs = ").append(outputs.toString()).append("\n");
        sb.append("shots = ").append(shots).append("\n");
        sb.append("error_range = ").append(errorRange == null ? "None" : errorRange).append("\n");
...
try:
    ...
    # Build QTCC preview
    preview_init = init_values
    if preview_init is None and len(test_suite) > 0 and len(test_suite[0]) == 2:
        maybe_inp = test_suite[0][0]
        if isinstance(maybe_inp, list):
            preview_init = maybe_inp
            
    if isinstance(preview_init, list):
        preview_init = [None if str(v).strip() == "" else v for v in preview_init]
        
    QTCC_preview = generateQTCC_stochastic(CuT, outputs, preview_init, inputs)
    qtcc_img = circuit_to_base64(QTCC_preview)
    
    # Run tests
    raw = []
    tol = float(error_range) if error_range is not None else None
    
    # Init inputs: use init_values if provided, else list of 0s
    ...
    # Normalize prepared (replace empty strings with None)
    ...
    for expected_out, expected_prob in test_suite:
        ...
        if isinstance(expected_prob, list):
            expected_prob = expected_prob[0] if len(expected_prob) > 0 else None
        try:
        ...
        if exp_float is not None:
            if 0.0 <= exp_float <= 1.0:
                expected_percent = round(exp_float * 100.0, 2)
            else:
                expected_percent = round(exp_float, 2)
